...
exports.index=function (data) {
	...
	else if(rq=="edit-plan"){
		edit_plan(data,function () {
			get_plans(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="remove-plan"){
		remove_plan(data,function () {
			get_plans(data,function () {
				db.ajres(data)
			})
		})
	...
	//OBJECTIVES
	else if(rq=="get-sp_objectives"){
		get_sp_objectives(data,function () {
			get_objectives(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-objective"){
		add_objective(data,function () {
			get_sp_objectives(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="get-sp_outcomes"){
		get_sp_outcomes(data,function () {

			get_outcomes(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-outcome"){
		add_outcome(data,function () {
			get_sp_outcomes(data,function () {
	...
	else if(rq=="get-sp-outcome-indicators"){
		get_sp_outcome_indicators(data,function () {

			get_outcome_indicators(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-sp-outcome-indicator"){
		add_sp_outcome_indicator(data,function () {
			get_sp_outcome_indicators(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="save-sp-outcome-indicator-target"){

		add_sp_outcome_indicator_target(0,data,function () {
			get_sp_outcome_indicators(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="get-sp_interventions"){
		get_sp_interventions(data,function () {

			get_interventions(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-intervention"){
		add_intervention(data,function () {
			get_sp_interventions(data,function () {
	...
	//OUTPUTS
	else if(rq=="get-sp_outputs"){
		get_sp_outputs(data,function () {

			get_outputs(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-output"){
		add_output(data,function () {
			get_sp_outputs(data,function () {
	...
	//output action
	else if(rq=="get-sp-output-actions"){
		get_sp_output_actions(data,function () {
			get_output_actions(data,function () {
				db.ajres(data)
			})
			
		})
	}
	else if(rq=="add-sp-output-action"){
		add_sp_output_actions(data,function () {
			get_sp_output_actions(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="add-sp-output-indicator"){
		add_sp_output_indicators(data,function () {
			get_sp_output_indicators(data,function () {
				db.ajres(data)
			})
		})
	}
	else if(rq=="save-sp-output-action-budget"){
		add_sp_output_action_budget(0,data,function () {
			get_sp_output_actions(data,function () {
				db.ajres(data)
			})
		})
  	...
  	//OUTPUT INDICATOR
  	//output indicator
	else if(rq=="get-sp-output-indicators"){
		
		get_sp_output_indicators(data,function () {
			get_output_indicators(data,function () {
				db.ajres(data)
			})
	...
	else if(rq=="save-sp-output-indicator-target"){
		add_sp_output_indicator_target(0,data,function () {
			get_sp_output_indicators(data,function () {
				db.ajres(data)
			})
		})
		...
		res.render("org_admin-plans",data)
	}
...
function get_plans(data,callback) {
	...
	var q="SELECT plans.id,prog_id,plan,prog_name,fy,status,DATE_FORMAT(plans.recorded,'%e %b %Y') AS recorded FROM plans,programmes WHERE userid=? AND plans.prog_id=programmes.id"
...
function submit_plan(data,callback) {
	var req=data.req;
	var plan_id=req.query.plan_id;
	var q="UPDATE plans SET status='Pending NPA Approval',submission_date=now() WHERE id=?"
	connection.query(q,[plan_id],function (err,rst) {
		if(err)
			return console.log(err)
		callback()
