...
function add_plan() {
	var hd=[{lb:"Programme",fn:"prog_id"},{lb:"Plan",fn:"plan"},{lb:"Financial Year",fn:"fy"}]
...
function load_plans(rst) {
	...
	for(var i=0;i<plans.length;i++){
		plans[i].plan_f="<button class='btn btn-danger' onclick=\"remove_plan("+i+")\"><i class='fa fa-trash'></i></button> <button class='btn btn-primary' onclick=\"edit_plan("+i+")\"><i class='fa fa-edit'></i></button> <span class='link' onclick=\"get_sp_objectives("+i+")\">"+plans[i].plan+"</span>"
		
	}
	var flds=[{lb:"#",ft:"serial"},{lb:"Plan",fn:"plan_f"},{lb:"FY",fn:"fy"},{lb:"Programme",fn:"prog_name"},{lb:"Creation Date",fn:"recorded"},{lb:"Status",fn:"status"}]
	
	gen_table(flds,plans,"plans-div","No plans created")
...
function load_preview(rst) {
	...
	for(var i=0;i<sp_objectives.length;i++){
		...
		div+=`<p>Programme Objective: ${sp_objectives[i].objective}</p>
			<p>Vote Objective: ${sp_objectives[i].org_objective}</p>`
...
function load_sp_objectives(rst) {
	...
	var flds=[{lb:"Objective",fn:"objective"},{lb:"Vote Objective",fn:"org_objective"},{lb:"",text:"Outcomes",ft:"btn",oc:"get_sp_outcomes"}]
	gen_table(flds,sp_objectives,"sp_objectives-div","No objectives added")
}

function add_objective() {
	adata.rq="add-objective"
	var data=prep_data([{lb:"Objective",ft:"sel",fn:"objective_id"},{lb:"Vote Objective",fn:"org_objective"}]);
	if(!data){return }
	data.plan_id=adata.plan_id;
	ajax_go(data,function (rst) {
		load_sp_objectives(rst)
...
function load_sp_outputs(rst) {
	...
	var flds=[{lb:"Output",fn:"output"},{lb:"",ft:"btn",text:"Indicators",oc:"get_sp_output_indicators"},{lb:"",ft:"btn",text:"VIAP Activities",oc:"get_piap_actions"}]
